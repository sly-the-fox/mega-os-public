...
Assess query complexity, select an appropriate research tier, decompose into MECE partitions when warranted, run parallel explorer agents, synthesize findings, and append a Codex+Parallax coherence reading.
...
## Coherence Reading
[Added by Phase 5.5 unless --no-codex]
...
## Phase 5.5: Codex + Parallax Coherence Reading (ALL TIERS)

**Skip this phase if `--no-codex` flag is set.**

After synthesis is complete (Phase 5, 5L) and before writing the final output file:

1. **Codex** reads the synthesized research and produces a harmonic perspective:
...
2. **Parallax** translates Codex's output into operational language:
...
[Codex's raw harmonic perspective — the pattern beneath the findings]
...
**Implementation:**
- For **light tier**: Run Codex and Parallax as two sequential Agent calls (read-only, no team needed) in the main context. Each agent receives only the synthesized document text (~1-2K tokens input each).
- For **standard/deep tiers**: Director spawns Codex and Parallax as additional brief Agent calls after synthesis. Or, if the team is still active, send the synthesis to two additional teammates.
...
/deep-research "competitive landscape" --no-codex
