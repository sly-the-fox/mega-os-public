...
4. Execute the Content Workflow:
   ...
   c. **Writer** — produce the first draft, applying `core/standards/writing-style.md`
   d. **Editor** — review for structure, citations, fact-checking, voice consistency
   e. **Writer** — revise based on editorial feedback (repeat d-e if needed)
   f. **Editor** — grant editorial approval
   g. **Polisher** — format for publication (invoke `/polish` on the approved draft)
   h. **Reviewer** — final quality check
5. Save the approved markdown draft to the output path.
