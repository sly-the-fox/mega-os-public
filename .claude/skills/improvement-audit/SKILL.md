...
description: Use when running the daily improvement audit — MECE-decomposed local codebase scan through auditor four-layer lens, improver distillation, and Coherence reading. Auto-rotates focus area by day of week.
...
| 4 | Shared Protocols | System rules, workflows, collaboration, escalation, handoff | `.claude/agents/shared/` (all files) |
...
| # | Partition | Scope | Key Files |
|---|-----------|-------|-----------|
| 1 | Agent Definitions | Knowledge agents — roles, boundaries, collaboration links | `.claude/agents/knowledge/` |
...
Customize these partitions to match your products:

| # | Partition | Scope | Key Files |
|---|-----------|-------|-----------|
| 1 | *Product A* | Spec, public repo, tests, marketing, adoption | `products/<product-a>/` |
| 2 | *Product B* | Spec, CLI, registry submissions, CI | `products/<product-b>/` |
| 3 | *Product C* | Spec, app code, landing page, deployment status | `products/<product-c>/` |
| 4 | Consulting & Templates | Service docs, template packages | `products/<consulting>/` |
| 5 | Revenue & Finance | Revenue tracker, financial records, pricing rationale | `business/finance/` |
...
| # | Partition | Scope | Key Files |
|---|-----------|-------|-----------|
| 1 | Business Agents | Business agents — strategist, marketer, seller, financier roles and boundaries | `.claude/agents/business/` |
| 2 | Strategy Alignment | Are strategy docs current? Do they align with active priorities? | `business/strategy/` |
| 3 | Sales & Pipeline | Freelance profile, client pipeline | `business/sales/`, `business/clients/` |
...
| # | Partition | Scope | Key Files |
|---|-----------|-------|-----------|
| 1 | Evolution Agents | Evolution agents — improver, evaluator, coherence, parallax definitions and boundaries | `.claude/agents/evolution/` |
...
| 4 | Coherence Integration | Metrics tracking, checkpoint invocations, anti-signal detection | `active/coherence-metrics.md` |
...
## Phase 7: Persist Findings to Audit Tracking

After writing the output file, persist ALL findings (High, Medium, and Low) to the canonical audit tracking file:

1. **Read** `active/audits.md` to check for existing entries.
2. **For each finding** from Phase 4 (all severities):
   - Generate an ID: `AUD-YYYYMMDD-N` (where N increments from 1)
   - Check if a finding with the same title/description already exists in `active/audits.md` — skip duplicates
   - Append to the matching severity section in `active/audits.md`:
     - HIGH findings -> `## Open — High` table
     - MEDIUM findings -> `## Open — Medium` table
     - LOW findings -> `## Open — Low` table
   - Table format (no Severity column — the section heading provides it):
     ```
     | AUD-YYYYMMDD-N | YYYY-MM-DD | improvement-audit ([Focus Area]) | [Finding title + concise evidence] | [Owner agent] | |
     ```
3. **Update the summary line** at top of file with new counts.
4. **Apply staleness policy:** Check for Low findings older than 14 days and Medium findings older than 30 days. Move expired items to `core/history/audit-archive.md` (append to the Resolved table with Status "Auto-archived" and today's date as Resolved Date).
5. **Print** to console: "Persisted N findings to active/audits.md (H: X, M: Y, L: Z). Auto-archived: N expired."

This ensures all findings survive the daily overwrite of `improvement-audit.md`. The staleness policy prevents unbounded growth of Medium/Low items.
...
- Findings persisted to audits.md by severity (H/M/L counts + auto-archived count)
- Output file path
