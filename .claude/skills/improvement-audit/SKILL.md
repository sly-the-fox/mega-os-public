...
## Phase 7: Persist High-Severity Findings to Audit Tracking

After writing the output file, persist HIGH-severity improvement proposals to the canonical audit tracking file:

1. **Read** `active/audits.md` to check for existing entries.
2. **For each HIGH-severity proposal** from Phase 4:
   ...
   - Append to the "Open Findings" table in `active/audits.md`:
     ```
     | AUD-YYYYMMDD-N | YYYY-MM-DD | improvement-audit ([Focus Area]) | [Finding title + evidence summary] | High | Open |
     ```
3. **Print** to console: "Persisted N high-severity findings to active/audits.md" (or "No new high-severity findings to persist")

This ensures high-severity findings survive the daily overwrite of `improvement-audit.md`.
...
- High-severity findings persisted to audits.md (count)
- Output file path
